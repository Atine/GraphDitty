...
def SalamiSSMFigure(num, cmap='magma_r'):
    ...
    # names = ['MFCCs', 'Tempogram', 'Chromas', 'Crema',
    #          'Fused MFCC/Chroma', 'Fused']
    ...
    for i, name in enumerate(names):
        ...
        L = np.asarray(mir_eval.hierarchy._meet(
            specintervals_hier[name],
            speclabels_hier[name],
            interval).todense())
    ...
    jam = ret['jam']
    annotators = []
    annotators.append(jam_annos_to_lists(
        jam.annotations[4], jam.annotations[5]))
    annotators.append(jam_annos_to_lists(
        jam.annotations[1], jam.annotations[2]))
    for k in range(2):
        plt.subplot(3, 3, 7+k)
        intervals, labels = annotators[k]
        L = np.asarray(mir_eval.hierarchy._meet(
            intervals, labels, interval).todense())
        times = interval*np.arange(L.shape[0])
...
def makeSNSPlot():
    ...
    res['LSD'] = scluster[
            ['MULTI_L-Precision', 'MULTI_L-Recall', 'MULTI_L-Measure']].values
    ...
    for i, plotname in enumerate(['L-Precision', 'L-Recall', 'L-Measure']):
        ...
        for name in names:
            ...
            if name == 'interanno':
                legend.append(
                    "interanno, mean = %.3g" % np.mean(interanno[:, i]))
            else:
                k = stats.ks_2samp(interanno[:, i], prl[:, i])[0]
                legend.append('%s, K=%.3g, Mean=%.3g' %
                              (name, k, np.mean(prl[:, i])))
        plt.legend(legend)
...
def makeTable():
    ...
    # L-Precision Mean, L-Precision KS, L-Recall Mean,
    # L-Recall KS, L-Score Mean, L-Score KS
    ...
    for name in ['MFCCs', 'Chromas', 'Tempogram', 'Crema',
                 'Fused MFCC/Chroma', 'Fused Tgram/Crema', 'Fused']:
        prl = res3[name]
        names.append(name)
    ...
    df = pandas.DataFrame(
            res, index=names,
            columns=['Mean Precision', 'KS Precision', 'Mean Recall',
                     'KS Recall', 'Mean L-Score', 'KS L-Score'])
