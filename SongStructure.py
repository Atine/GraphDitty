...
def plotFusionResults(Ws, vs, alllabels, times, win_fac,
    ...
    """
    Show a plot of different adjacency matrices and their associated
    eigenvectors and cluster labels, if applicable
    Parameters
    ----------
    ...
    vs: Dictionary of string:ndarray(N, k)
        ...
        If there is not a key for a particular adjacency matrix type,
        it isn't plotted
    ...
    win_fac: int
        ...
        If negative, beat tracking has been done, and the intervals are
        possibly non-uniform. This means that a mesh plot will be necessary
    Returns
    -------
    ...
    for i, name in enumerate(Ws):
        ...
        if name in alllabels:
            ...
            if time_uniform:
                ...
                plt.pcolormesh(np.arange(labels.shape[1]+1), times, labels,
                               cmap='tab20b')
                plt.gca().invert_yaxis()
            plt.axis('off')
            plt.title("Clusters")
    ...
    if len(labels_hier) > 0:
        for k in range(2):
            ...
            for a in labels_hier[k]:
                ...
                if a not in labelsdict:
                    labelsdict[a] = len(labelsdict)
                labels.append(labelsdict[a])
            labels = np.array(labels)
            plt.pcolormesh(np.arange(2),
                           intervals_hier[k][:, 0],
                           np.concatenate(
                               (labels[:, None], labels[:, None]), 1),
                           cmap='tab20b')
...
def getFusedSimilarity(filename, sr, hop_length,
                       win_fac, wins_per_block,
                       K, reg_diag, reg_neighbs,
                       niters, do_animation,
                       plot_result, do_crema=True):
    """
    ...
    win_fac: int
        ...
        If negative, then do beat tracking, and subdivide by |win_fac|
        times within each beat
    wins_per_block: int
    ...
    Returns
    -------
    {'Ws': An dictionary of weighted adjacency matrices for individual
                    features and the fused adjacency matrix,
    ...
    if MANUAL_AUDIO_LOAD:
        # subprocess.call([FFMPEG_BINARY, "-i", filename,
        #                  "-ar", "%i" % sr, "-ac", "1", "%s.wav" % filename])
        sr, y = sio.wavfile.read(filename)
        y = y/2.0**15
    ...
    # 3) Tempograms
    #  Use a super-flux max smoothing of 5 frequency bands
    #  in the oenv calculation
    SUPERFLUX_SIZE = 5
    ...
    WFused = doSimilarityFusionWs(
            Ws, K=pK, niters=niters,
    ...
    WsDict['Fused MFCC/Chroma'] = doSimilarityFusionWs(
            Ws[0:2], K=pK, niters=niters,
            reg_diag=reg_diag, reg_neighbs=reg_neighbs)
    if do_crema:
        # Do fusion with tempograms and Crema if Crema is available
        WsDict['Fused Tgram/Crema'] = doSimilarityFusionWs(
                Ws[2::], K=pK, niters=niters,
                reg_diag=reg_diag, reg_neighbs=reg_neighbs)
        # Do fusion with MFCC and Crema
        WsDict['Fused MFCC/Crema'] = doSimilarityFusionWs(
                [Ws[0], Ws[-1]], K=pK, niters=niters,
                reg_diag=reg_diag, reg_neighbs=reg_neighbs)
        # Do fusion with MFCC, Chroma, and Crema
        WsDict['Fused MFCC/Chroma/Crema'] = doSimilarityFusionWs(
                [Ws[0], Ws[1], Ws[-1]], K=pK, niters=niters,
                reg_diag=reg_diag, reg_neighbs=reg_neighbs)
    if plot_result:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--do_animation', type=int, default=0,
                        help='Save plots of iterations to disk')
    parser.add_argument('--plot_result', type=int, default=1,
                        help='Plot the result of fusion')
    parser.add_argument('--sr', type=int, default=22050,
                        help='Sample rate to use')
    parser.add_argument('--hop_length', type=int, default=512,
                        help="Hop Size in samples")
    parser.add_argument('--win_fac', type=int, default=10,
                        help="Number of windows to average in a frame. "
                             "If negative, then do beat tracking, and "
                             "subdivide by |win_fac| times within each beat")
    parser.add_argument('--wins_per_block', type=int, default=20,
                        help="Number of frames to stack in sliding "
                             "window for every feature")
    parser.add_argument('--K', type=int, default=5,
                        help="Number of nearest neighbors in similarity "
                             "network fusion.  If -1, then autotune to "
                             "sqrt(N) for an NxN similarity matrix")
    parser.add_argument('--reg_diag', type=float, default=1.0,
                        help="Regularization for self-similarity promotion")
    parser.add_argument('--reg_neighbs', type=float, default=0.5,
                        help="Regularization for direct neighbor "
                             "similarity promotion")
    parser.add_argument('--niters', type=int, default=3,
                        help="Number of iterations in similarity "
                             "network fusion")
    parser.add_argument('--neigs', type=int, default=8,
                        help="Number of eigenvectors in the graph Laplacian")
    parser.add_argument('--matfilename', type=str, default="out.mat",
                        help="Name of the .mat file to which to save the "
                             "results")
    parser.add_argument('--jsonfilename', type=str, default=None,
                        help="Name of the .json file to which to save "
                             "results for viewing in the GUI")
    parser.add_argument('--diffusion_znormalize', type=int, default=1,
                        help="Whether to perform Z-normalization with "
                             "diffusion maps to spread things out more")

    opt = parser.parse_args()
    if opt.jsonfilename is None:
        opt.jsonfilename = opt.filename.replace('.wav', '.json')

    res = getFusedSimilarity(
            opt.filename,
            sr=opt.sr,
            hop_length=opt.hop_length,
            win_fac=opt.win_fac,
            wins_per_block=opt.wins_per_block,
            K=opt.K,
            reg_diag=opt.reg_diag,
            reg_neighbs=opt.reg_neighbs,
            niters=opt.niters,
            do_animation=opt.do_animation,
            plot_result=opt.plot_result,
            do_crema=False)

    sio.savemat(opt.matfilename, res)
    saveResultsJSON(opt.filename, res['times'], res['Ws'],
                    opt.neigs, opt.jsonfilename, opt.diffusion_znormalize)
